    ...
    return (<div className='Header' >
        <div className='Container'>
            ...
            <form onSubmit={(event) => handleSubmit(event)} className="SearchBar">
                <input type="text" placeholder="Nunca dejes de buscar" value={query} onChange={(event) => setQuery(event.target.value)} className="SearchInput" />
                {query !== '' ?
                    <button type="reset" className="IconButton" onClick={(event) => clearForm(event) }>
